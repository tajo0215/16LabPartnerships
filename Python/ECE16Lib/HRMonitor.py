...
class HRMonitor:
	...
	def train(self, directory):
		...
		for epoch in range(1):
			for exclude in subjects:
				...
				for trial in range(1, 11):
					t, ppg, hr, fs_est = self.get_data(directory, exclude, trial)
					...
					ppg = filt.moving_average(ppg, 5)
					ppg = filt.gradient(ppg)
					validation_data = filt.normalize(ppg)

					labels = model.predict(validation_data.reshape(-1, 1))
				...
				print(f"Epoch {epoch + 1}: RMSE = {rmse:.4f}, r = {r:.4f}")
	...
	def predict(self, filename):
		...
		data = np.array(self.__filtered).reshape(-1, 1)
		"""
